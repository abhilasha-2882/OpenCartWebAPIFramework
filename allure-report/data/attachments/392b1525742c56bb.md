        ...
        - generic [ref=e87]:
          ...
          - generic [ref=e91]:
            ...
            - generic [ref=e95]:
              ...
              - textbox "Password" [ref=e97]: pw1234
...
```ts
  ...
  8  |     })
  9  | 
  10 | test('login page title test',async({loginpage})=>{
  11 | 
  12 | const pagetitle=await loginpage.getLoginPageTitle();
  13 | console.log(pagetitle);
  14 | expect(pagetitle).toBe('Account Login');
  15 | });
  16 | 
  17 | test('forgotpassword link exists test',async({loginpage})=>{
  18 | 
  19 | const isexist=await loginpage.isForgetPwdLinkExist(); 
  20 | expect(isexist).toBeTruthy();
  21 | });
  22 | 
  23 | test('user is able to login to app test',async({loginpage,homepage})=>{
  24 | 
  25 | await loginpage.doLogin('pwtestbatch@open.com','pw1234');
> 26 | expect(await homepage.isLogoutLinkExist()).toBeTruthy();
     |                                            ^ Error: expect(received).toBeTruthy()
  27 | expect (await homepage.getHomePageTitle()).toBe('My Account');
  28 | 
  29 | });
  30 | 
  31 | // to integrate with allure report command is as below
  32 | //npm install --save-dev allure-playwright allure-commandline
```