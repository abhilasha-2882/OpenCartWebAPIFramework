...
- Name: RegisterPage.spec.ts >> invalid login test with -Gauransh-Varshney
...
Received: [Function getRegisterPagheading]
```
          ...
          - group "Your Personal Details" [ref=e82]:
            ...
            - generic [ref=e92]:
              ...
              - textbox "* E-Mail" [ref=e95]:
                - /placeholder: E-Mail
                - text: gauransh.varshney@gmail.com
            - generic [ref=e96]:
...
```ts
  ...
  4  | 
  5  |  test.beforeEach(async({registerpage})=>{
  6  |    
  7  |      await registerpage.goToRegisterPage(); 
  8  |    
  9  |     })
  10 | 
  11 | let testData=csvHelper.readCsv('src/data/RegisterData.csv');
  12 | 
  13 | for(let row of testData)
  14 | {
  15 |     test(`invalid login test with -${row.firstname}-${row.lastname}`,async({registerpage})=>{
  16 |     await registerpage.doRegister(row.firstname!,row.lastname!,row.emailid!,row.telephone!,row.password!,row.confirmpassword!);
> 17 |     expect(registerpage.getRegisterPagheading).toBe('Your Account Has Been Created!'); 
     |                                                ^ Error: expect(received).toBe(expected) // Object.is equality
  18 | });
  19 | 
  20 | };
